...
void CsweepMineMFCView::SetMineMap(UINT8 mineMapSize, UINT16 mineCnt, UINT16 grade) {
	...
	m_heightOffset = (UINT8)(40 + m_bitMapInfo.bmHeight);
...
void CsweepMineMFCView::ShowInfo(CDC *pDc) {
	...
	CDC dc;
	CFont font;
	font.CreatePointFont(100, "宋体", NULL);
	CFont *pOldFont = pDC->SelectObject(&font);
	COLORREF clr = pDC->SetTextColor(0);
	CString str;
	if (m_mineCnt - m_tagCnt <0)
	{
		str.Format("标记的雷数超过设定值");
		MessageBox(str);		
	}
	str.Format("剩余雷数：%d                   时间：%d:%d:%d  ", m_mineCnt-m_tagCnt, m_time / 3600, m_time % 3600 / 60, m_time % 3600 % 60);
